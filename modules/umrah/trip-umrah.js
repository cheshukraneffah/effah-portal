            ...
            <div class="bg-white rounded-3xl shadow-2xl p-6 md:p-8 max-w-md w-full border border-slate-100 animate-in fade-in zoom-in duration-150">
                ...
                <form onsubmit="submitNewTripRecord(event)" class="space-y-4 text-xs font-medium text-slate-700">
                    ...
                    <div class="flex items-center space-x-3 pt-3">
                        <button type="button" onclick="closeNewTripModal()" class="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-2.5 rounded-xl transition">Batal</button>
                        <button type="submit" class="flex-1 bg-slate-900 hover:bg-black text-white font-bold py-2.5 rounded-xl transition shadow-xs">Cipta Trip</button>
                    </div>
                </form>
            </div>
        </div>
...
function renderTripSidebarList(records) {
    ...
    records.forEach(rec => {
        ...
        card.innerHTML = `
            ...
            <div class="mt-2">
                <span class="bg-amber-100/80 text-amber-900 border border-amber-200/60 text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wide">${airline}</span>
            </div>
        `;
                        ...
                        ` : currentTripJemaahList.map((j, idx) => {
                            ...
                            return `
                                <tr class="hover:bg-slate-50/80 transition">
                                    ...
                                    <td class="p-3">${pic ? `<img src="${pic}" onclick="openTripPreviewModal('${pic}', '${(jf['NAME']||'').replace(/'/g, '')} - PICTURE')" class="w-9 h-9 rounded-lg object-cover border border-slate-200 cursor-pointer hover:scale-110 transition" title="Click to preview">` : `<div class="w-9 h-9 bg-slate-100 rounded-lg flex items-center justify-center text-slate-400"><i class="fa-solid fa-user"></i></div>`}</td>
                                    <td class="p-3">${passCopy ? `<button onclick="openTripPreviewModal('${passCopy}', '${(jf['NAME']||'').replace(/'/g, '')} - PASSPORT COPY')" class="text-sky-600 hover:text-sky-800 underline font-semibold flex items-center text-xs"><i class="fa-solid fa-file-pdf mr-1"></i> View Copy</button>` : `<span class="text-slate-300">-</span>`}</td>
...
function getGenderBreakdown(){
  ...
  currentTripJemaahList.forEach(j=>{
    ...
    if(m) ageYears=parseInt(m[1]); else ageYears=parseFloat(ageStr)||0;
    const isKid=ageYears>0&&ageYears<12;
    if(g==='MALE'){ if(isKid) maleKids++; else male++; }
    else if(g==='FEMALE'){ if(isKid) femaleKids++; else female++; }
  });
...
function buildTripManifestHTML(includeButtons=true){
  ...
  const rows=currentTripJemaahList.map((j,idx)=>{
    ...
    const name=jf['NAME']||'';
    return `<tr><td>${idx+1}</td><td class="name-col">${name}</td><td>${gender}</td><td>${passport}</td><td>${dateIssue}</td><td>${dateExpire}</td><td>${dob}</td><td>${age}</td><td>${ic}</td><td>${nationality}</td></tr>`;
  }).join('');
  return `
<!DOCTYPE html>
<html>
...
<style>
  *{font-family:Arial, sans-serif; box-sizing:border-box;}
  ...
  @media print{ body{padding:0;} .no-print{display:none !important;} @page{size:A4 landscape; margin:10mm;} }
</style>
</head>
<body>
  <div id="manifestContent">
    <div class="header-card">
      ...
      <div class="gender-row">
        ...
        <span>👨 Male: ${gb.male}</span>
        <span>👩 Female: ${gb.female}</span>
        <span>🧒 Male Kids: ${gb.maleKids}</span>
        <span>👧 Female Kids: ${gb.femaleKids}</span>
      </div>
    </div>
    <table>
      <thead><tr><th style="width:28px;">#</th><th>Nama Jemaah</th><th>Gender</th><th>Passport No.</th><th>Date Issue</th><th>Date Expire</th><th>DOB</th><th>Age</th><th>IC No.</th><th>Nationality</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>
  </div>
  ${includeButtons ? `<div class="no-print" style="margin-top:20px; text-align:center;"><button onclick="window.print()" style="background:#0f172a; color:#fff; padding:10px 24px; border-radius:8px; border:none; font-weight:700; cursor:pointer; margin-right:10px;">🖨️ Print</button><button onclick="window.close()" style="background:#f1f5f9; color:#334155; padding:10px 24px; border-radius:8px; border:1px solid #cbd5e1; font-weight:700; cursor:pointer;">Tutup</button></div><script>window.onafterprint=function(){ window.close(); }; window.addEventListener('keydown', function(e){ if(e.key==='Escape'){ window.close(); } });<\/script>` : ''}
</body>
</html>`;
}

function printTripManifest(){
  const html=buildTripManifestHTML(true);
  ...
  // Fallback: if user closes print dialog, still close after 1s? Actually onafterprint handles both
...
window.openTripPreviewModal = function(url, title){
  ...
  const img = modal.querySelector('#previewImage') || document.getElementById('previewImage');
  const pdf = modal.querySelector('#previewPdf') || document.getElementById('previewPdf');
  const titleEl = modal.querySelector('#previewTitle') || document.getElementById('previewTitle');
  const dlBtn = modal.querySelector('#downloadAttachmentBtn') || document.getElementById('downloadAttachmentBtn');
  ...
  if(isPdf){ if(pdf){ pdf.src = url; pdf.classList.remove('hidden'); } } else { if(img){ img.src = url; img.classList.remove('hidden'); } }
...
window.closePreviewModal = function(){
  document.querySelectorAll('#attachmentPreviewModal').forEach(m=>{
...
window.openPreviewModal = function(url, title){ return window.openTripPreviewModal(url, title); };
