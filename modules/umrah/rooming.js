...
// V113 - FIX HOTEL PAKEJ SAME AS JEMAAH COLOR + BOARD CLEAR

const PAKEJ_ORDER_V113 = ['JIMAT EKONOMI','JIMAT STANDARD','JIMAT PREMIUM','EKONOMI LITE','EKONOMI','STANDARD','PREMIUM','PREMIUM PLUS'];
const PAKEJ_COLORS_V113 = {
...
function closeAllDropdowns_V113(){
...
window.toggleBoardDropdown = toggleBoardDropdown_V113;
...
function toggleBoardMulti_V113(jId, boardVal){
  try{
    const jRec = (window.allRoomingJemaah||[]).find(j=>j.id===jId);
    if(jRec){
      if(!jRec.fields) jRec.fields={};
      const fb = getBoardFallback_V113(jId);
      ...
      setBoardFallback_V113(jId, jRec.fields['BOARD BASIS']);
      const drop = document.getElementById('boardDrop-'+jId);
      if(drop){
      ...
      document.querySelectorAll(`button[onclick*="toggleBoardDropdown_V113('${jId}')"]`).forEach(btn=>{
      ...
      if(base&&pat){
        fetch(`https://api.airtable.com/v0/${base}/JEMAAH/${jId}`,{
          ...
          console.log('V113 board toggle', r.status, jRec.fields['BOARD BASIS']);
...
window.toggleBoardMulti = toggleBoardMulti_V113;

function clearBoardMulti_V113(jId){
  console.log('V113 clearBoardMulti', jId);
  try{
    const jRec = (window.allRoomingJemaah||[]).find(j=>j.id===jId);
    ...
    if(base&&pat){
      ...
      fetch(`https://api.airtable.com/v0/${base}/${table}/${idToPatch}`,{
        method:'PATCH', headers:{'Authorization':`Bearer ${pat}`,'Content-Type':'application/json'},
        body: JSON.stringify({fields:{'BOARD BASIS': []}})
      }).then(r=>console.log('V113 clear board', r.status));
    }
  }catch(e){ console.error(e); }
}
window.clearBoardMulti = clearBoardMulti_V113;

// PAKEJ JEMAAH
function togglePakejDropdown_V113(jId){
...
window.togglePakejDropdown = togglePakejDropdown_V113;
function closePakejDropdown_V113(jId){
  const drop = document.getElementById('pakejDrop-'+jId);
  if(drop) drop.classList.add('hidden');
  setTimeout(()=>{ if(typeof renderNamelist==='function') renderNamelist(); }, 10);
}
window.closePakejDropdown = closePakejDropdown_V113;
function selectPakej_V113(jId, pakej){
  const jRec = (window.allRoomingJemaah||[]).find(j=>j.id===jId);
  if(jRec) jRec.fields['PAKEJ']=pakej;
  const drop = document.getElementById('pakejDrop-'+jId);
  if(drop) drop.classList.remove('hidden');
  const btn = document.querySelector(`button[onclick*="togglePakejDropdown_V113('${jId}')"]`);
  if(btn){
    const color = PAKEJ_COLORS_V113[pakej] || '#FFFFFF';
  ...
  if(base&&pat&&jRec){
    ...
    }).then(r=>console.log('V113 pakej saved', r.status));
  }
}
window.selectPakej = selectPakej_V113;
function clearPakej_V113(jId){
  const jRec = (window.allRoomingJemaah||[]).find(j=>j.id===jId);
...
window.clearPakej = clearPakej_V113;

// HOTEL PAKEJ - FIX TO USE SAME COLOR DROPDOWN AS JEMAAH
function toggleHotelPakejDropdown_V113(roomId){
...
window.toggleHotelPakejDropdown = toggleHotelPakejDropdown_V113;

function closeHotelPakejDropdown_V113(roomId){
  const drop = document.getElementById('hotelPakejDrop-'+roomId);
  if(drop) drop.classList.add('hidden');
  setTimeout(()=>{ if(typeof renderRoomingGrid==='function') renderRoomingGrid(); }, 10);
}
window.closeHotelPakejDropdown = closeHotelPakejDropdown_V113;

function selectHotelPakej_V113(roomId, pakej){
  const room = (window.allRoomingRecords||[]).find(r=>r.id===roomId);
  if(room) room.fields['PAKEJ / HOTEL']=pakej;
  const drop = document.getElementById('hotelPakejDrop-'+roomId);
  if(drop) drop.classList.remove('hidden');
  const btn = document.querySelector(`button[onclick*="toggleHotelPakejDropdown_V113('${roomId}')"]`);
  if(btn){
    const color = PAKEJ_COLORS_V113[pakej] || '#FFFFFF';
    btn.style.background=color; btn.style.borderColor=color;
  ...
  if(base&&pat&&room){
    ...
    }).then(r=>console.log('V113 hotel pakej saved', r.status, pakej));
  }
}
window.selectHotelPakej = selectHotelPakej_V113;

function clearHotelPakej_V113(roomId){
  const room = (window.allRoomingRecords||[]).find(r=>r.id===roomId);
...
window.clearHotelPakej = clearHotelPakej_V113;

// Outside click auto collapse
document.removeEventListener('click', window._v113Outside);
window._v113Outside = function(e){
  const isBoardBtn = e.target.closest('button[onclick*="toggleBoardDropdown_V113"]');
  const isPakejBtn = e.target.closest('button[onclick*="togglePakejDropdown_V113"]');
  const isHotelBtn = e.target.closest('button[onclick*="toggleHotelPakejDropdown_V113"]');
...
document.addEventListener('click', window._v113Outside);

// Override renderNamelist with pakej color dropdown
const origRenderNamelist_V113 = window.renderNamelist;
window.renderNamelist = function(){
  try{
    (window.allRoomingJemaah||[]).forEach(j=>{
      if(!j.fields) j.fields={};
      j.fields._jemaahId = j.id;
      const fb = getBoardFallback_V113(j.id);
  ...
  cont.innerHTML=filtered.map((r,i)=>{
    ...
    const boardCheckboxes = boardOptions.map(opt=>{
      const checked=fbArr.includes(opt);
      return `<label class="flex items-center gap-1.5 px-2 py-1 hover:bg-slate-50 rounded text-[10px] cursor-pointer" onclick="event.stopPropagation()"><input type="checkbox" ${checked?'checked':''} onchange="toggleBoardMulti_V113('${r.id}','${opt}')" class="w-3 h-3 accent-[#7A0C2E]"> ${opt}</label>`;
    }).join('');
    ...
    const pkColor = PAKEJ_COLORS_V113[pk] || '#FFFFFF';
    const pkDisplay = pk || '-';
    const pakejDropdown = `<div class="relative"><button onclick="event.stopPropagation(); togglePakejDropdown_V113('${r.id}')" class="w-full text-[9px] border rounded-full px-2.5 py-1.5 font-bold text-left flex items-center justify-between" style="background:${pkColor}; border-color:${pkColor}; color:#1F2937"><span class="truncate">${pkDisplay}</span><span>▼</span></button><div id="pakejDrop-${r.id}" class="hidden absolute z-[9999] mt-1 w-48 bg-white border border-slate-300 rounded-xl shadow-2xl p-1" onclick="event.stopPropagation()">${PAKEJ_ORDER_V113.map(o=>{
      const c = PAKEJ_COLORS_V113[o];
      const isSel = pk===o;
      return `<div onclick="selectPakej_V113('${r.id}','${o}')" class="flex items-center gap-2 px-2.5 py-1.5 hover:bg-slate-50 cursor-pointer text-[11px] rounded-lg ${isSel?'bg-slate-100 font-bold':''}"><span class="w-4 h-4 rounded-full flex items-center justify-center text-[9px] border" style="background:${c}; border-color:${c}">${isSel?'✓':''}</span> ${o} <span class="ml-auto text-slate-400">${isSel?'✓':'×'}</span></div>`;
    }).join('')}<div class="flex justify-between gap-1 mt-1 pt-1 border-t bg-white"><button onclick="clearPakej_V113('${r.id}'); event.stopPropagation();" class="text-[9px] px-3 py-1 rounded-full bg-slate-100">Clear</button><button onclick="closePakejDropdown_V113('${r.id}'); event.stopPropagation();" class="text-[9px] px-3 py-1 rounded-full bg-[#7A0C2E] text-white">OK</button></div></div></div>`;
    const trChecked = typeof isTrainChecked==='function'? isTrainChecked(r.fields) : false;
    return `<div ${drag} class="grid grid-cols-12 items-center px-1.5 py-1.5 text-[11px] border-b border-slate-50 ${rowCls}">
      <div class="col-span-1 text-slate-400 text-[10px]">${String(i+1).padStart(2,'0')}</div>
      <div class="col-span-3 font-medium truncate text-[10px] ${assignedInLoc?'text-slate-500 italic':''}" title="${name}">${name}</div>
      <div class="col-span-2 flex items-center gap-0.5 relative">
        <div class="relative w-full">
          <button onclick="event.stopPropagation(); toggleBoardDropdown_V113('${r.id}')" class="text-[8px] border rounded-full px-2 py-1 font-bold ${fbCls} outline-none w-full truncate text-left flex items-center justify-between bg-white">
            <span class="truncate">${fbDisplay}</span><span class="ml-1">▼</span>
          </button>
          <div id="boardDrop-${r.id}" class="hidden absolute left-0 top-full mt-1 w-[190px] bg-white border border-slate-200 rounded-xl shadow-xl z-[9999] p-1" onclick="event.stopPropagation()">
            ${boardCheckboxes}
            <div class="border-t border-slate-100 mt-1 pt-1 flex justify-between">
              <button onclick="clearBoardMulti_V113('${r.id}'); event.stopPropagation();" class="text-[8px] px-2 py-0.5 rounded-full bg-slate-100">Clear</button>
...
// Override renderRoomingGrid to include hotel pakej color dropdown
const origRenderRoomingGrid_V113 = window.renderRoomingGrid;
window.renderRoomingGrid = function(){
  if(origRenderRoomingGrid_V113) origRenderRoomingGrid_V113();
  setTimeout(()=>{
