      ...
      <div class="w-full lg:w-[52%] bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col overflow-hidden">
        ...
        <div class="px-2.5 py-1.5 bg-slate-50/70 border-b border-slate-200 grid grid-cols-12 text- font-bold text-slate-500 tracking-wider">
          ...
          <div class="col-span-3 flex items-center gap-1 cursor-pointer hover:text-[#7A0C2E] select-none" onclick="toggleSortNama()" title="Klik untuk sort A-Z / Z-A">
            <span id="headerNamaJemaah">NAMA JEMAAH</span>
            <span id="sortIcon" class="text-">↕</span>
          </div>
          <div class="col-span-2 text-center">FULLBOARD</div><div class="col-span-1 text-center">TRAIN</div><div class="col-span-3 text-center">INSURAN</div><div class="col-span-1 text-center">PAKEJ</div><div class="col-span-1 text-center">+</div>
        </div>
...
function populateRoomingTripDropdown(){
  ...
  if(trips.length===0){
    if(!sel.dataset.retries) sel.dataset.retries='0';
    let retries=parseInt(sel.dataset.retries||'0');
    if(retries<20){
      sel.dataset.retries=retries+1;
      setTimeout(()=>{
        if(typeof fetchTripUmrahData==='function' && retries%3===0) fetchTripUmrahData();
        if(typeof loadAllTripData==='function' && retries%3===0) loadAllTripData();
        populateRoomingTripDropdown();
      }, 800);
    } else {
      sel.innerHTML='<option value="">Tiada trip - refresh page</option>';
    }
    return;
  }
  ...
  if(sel.value && window.selectedTripRecord?.id!==sel.value){
    const found=trips.find(t=>t.id===sel.value);
...
function toggleSortNama(){
  if(!roomingSortActive){ roomingSortActive=true; roomingSortDir='asc'; }
  else { roomingSortDir = roomingSortDir==='asc'? 'desc' : 'asc'; }
...
async function removeJemaahFromAnyRoom(jId){
  ...
  for(const rec of roomsInLoc){ await updateRoomField(rec.id,'JEMAAH',(rec.fields['JEMAAH']||[]).filter(id=>id!==jId),false); }
...
async function deleteRoom(roomId,roomName){
  if(!confirm(`Adakah anda pasti ingin memadamkan bilik ${roomName}?`)) return;
  const base=window.AIRTABLE_BASE_ID||localStorage.getItem('effah_api_base')||localStorage.getItem('effah_base_id'); const pat=window.AIRTABLE_PAT||localStorage.getItem('effah_api_pat');
  try{
...
async function submitNewRoom(){
  ...
  try{
    ...
    if(newRec.id){ allRoomingRecords.push(newRec); closeNewRoomModal(); renderRoomingGrid(); renderLocationTabs(); renderNamelist(); renderStaffList(); }
...
async function submitCopyRooms(){
  ...
  if(created>0) alert(`Berjaya menyalin ${created} bilik dari ${src} ke ${activeLocation}.` + (failed>0?` ${failed} gagal.`:''));
  else alert('Gagal menyalin bilik.');
}
...
function generateRoomingPrint(){
  ...
  let namelistRows=allRoomingJemaah.map((j,idx)=>{
    ...
    return `<tr><td>${idx+1}</td><td>${name}</td><td><span style="${fbStyle}">${board}</span></td><td><span style="${trainStyle}">${train}</span></td><td><span style="${pakejStyle}">${pakej}</span></td><td>${insBadges}</td></tr>`;
  }).join('');
