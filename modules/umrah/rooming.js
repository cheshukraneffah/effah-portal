...
// V114 - FIX HOTEL PAKEJ BUTTON NOT UPDATING + BOARD 403 FULL ERROR + FALLBACK

const PAKEJ_ORDER_V114 = ['JIMAT EKONOMI','JIMAT STANDARD','JIMAT PREMIUM','EKONOMI LITE','EKONOMI','STANDARD','PREMIUM','PREMIUM PLUS'];
const PAKEJ_COLORS_V114 = {
...
function closeAllDropdowns_V114(){
...
// BOARD FALLBACK
function getBoardFallback_V114(jId){
  try{ const raw = localStorage.getItem('effah_board_fallback_'+jId); if(raw) return JSON.parse(raw); }catch(e){}
  return null;
}
function setBoardFallback_V114(jId, arr){
  try{ localStorage.setItem('effah_board_fallback_'+jId, JSON.stringify(arr)); }catch(e){}
}

function toggleBoardDropdown_V114(jId){
  const drop = document.getElementById('boardDrop-'+jId);
  if(!drop) return;
  const isHidden = drop.classList.contains('hidden');
  closeAllDropdowns_V114();
...
window.toggleBoardDropdown = toggleBoardDropdown_V114;
...
async function toggleBoardMulti_V114(jId, boardVal){
  try{
    const jRec = (window.allRoomingJemaah||[]).find(j=>j.id===jId);
    if(jRec){
      if(!jRec.fields) jRec.fields={};
      const fb = getBoardFallback_V114(jId);
      if((!jRec.fields['BOARD BASIS'] || jRec.fields['BOARD BASIS'].length===0) && fb && fb.length){
        jRec.fields['BOARD BASIS']=[...fb];
      }
      ...
      setBoardFallback_V114(jId, jRec.fields['BOARD BASIS']);
      ...
      document.querySelectorAll(`button[onclick*="toggleBoardDropdown_V114('${jId}')"]`).forEach(btn=>{
      ...
      if(base&&pat){
        ...
        console.log('V114 board toggle', res.status, jRec.fields['BOARD BASIS'], 'response', json);
        if(res.status===403){
          console.error('403 FULL ERROR:', json);
          console.error('Error type:', json.error?.type, 'Message:', json.error?.message);
          if(json.error?.message?.includes('permission') || json.error?.message?.includes('Permission')){
            alert('BOARD BASIS 403: Field permission locked. Pergi Airtable → BOARD BASIS header → Edit field permissions → Set to Everyone can edit. Buat masa ni board 2 disimpan dalam localStorage fallback.');
          }
        }
        if(res.ok){ try{ localStorage.removeItem('effah_board_fallback_'+jId); }catch(e){} }
      }
    } else {
      ...
      if(sRec){
        ...
        if(base&&pat&&sRec.airtableId){
          fetch(`https://api.airtable.com/v0/${base}/STAFF%20LIST%20%28ROOMING%29/${sRec.airtableId}`,{
            method:'PATCH', headers:{'Authorization':`Bearer ${pat}`,'Content-Type':'application/json'},
            body: JSON.stringify({fields:{'BOARD BASIS': sRec.boardBasis}, typecast: true})
          }).then(r=>console.log('V114 staff board', r.status));
        }
      }
    }
  }catch(e){ console.error(e); }
}
window.toggleBoardMulti = toggleBoardMulti_V114;

function clearBoardMulti_V114(jId){
  console.log('V114 clearBoardMulti', jId);
  try{
    const jRec = (window.allRoomingJemaah||[]).find(j=>j.id===jId);
    if(jRec){ if(!jRec.fields) jRec.fields={}; jRec.fields['BOARD BASIS']=[]; }
    const sRec = (window.staffList||[]).find(s=>s.id===jId||s.airtableId===jId);
    if(sRec){ sRec.board = sRec.train ? ['TRAIN'] : []; sRec.boardBasis=[]; try{ localStorage.setItem('effah_staff_board_'+jId, JSON.stringify(sRec.board)); }catch(e){} }
    setBoardFallback_V114(jId, []);
    const drop = document.getElementById('boardDrop-'+jId);
    if(drop){
    ...
    document.querySelectorAll(`button[onclick*="toggleBoardDropdown_V114('${jId}')"]`).forEach(btn=>{
      ...
      btn.className='text-[8px] border rounded-full px-2 py-1 font-bold bg-white border-dashed border-slate-300 text-slate-400 outline-none w-full truncate text-left flex items-center justify-between';
    });
    const base=window.AIRTABLE_BASE_ID||localStorage.getItem('effah_api_base');
    const pat=window.AIRTABLE_PAT||localStorage.getItem('effah_api_pat');
    if(base&&pat){
      ...
      fetch(`https://api.airtable.com/v0/${base}/${table}/${idToPatch}`,{
        ...
        console.log('V114 clear board', r.status, j);
      });
    }
  }catch(e){ console.error(e); }
}
window.clearBoardMulti = clearBoardMulti_V114;

// PAKEJ JEMAAH - FIX
function togglePakejDropdown_V114(jId){
...
window.togglePakejDropdown = togglePakejDropdown_V114;
function closePakejDropdown_V114(jId){
  const drop = document.getElementById('pakejDrop-'+jId);
  if(drop) drop.classList.add('hidden');
  setTimeout(()=>{ if(typeof renderNamelist==='function') renderNamelist(); }, 10);
}
window.closePakejDropdown = closePakejDropdown_V114;
function selectPakej_V114(jId, pakej){
  const jRec = (window.allRoomingJemaah||[]).find(j=>j.id===jId);
  if(jRec) jRec.fields['PAKEJ']=pakej;
  const drop = document.getElementById('pakejDrop-'+jId);
  if(drop) drop.classList.remove('hidden');
  const btn = document.querySelector(`button[onclick*="togglePakejDropdown_V114('${jId}')"]`);
  if(btn){
    const color = PAKEJ_COLORS_V114[pakej] || '#FFFFFF';
  ...
  if(base&&pat&&jRec){
    ...
    }).then(r=>console.log('V114 pakej saved', r.status));
  }
}
window.selectPakej = selectPakej_V114;
function clearPakej_V114(jId){
  const jRec = (window.allRoomingJemaah||[]).find(j=>j.id===jId);
...
window.clearPakej = clearPakej_V114;

// HOTEL PAKEJ - FIX BUTTON NOT UPDATING BUG
function toggleHotelPakejDropdown_V114(roomId){
...
window.toggleHotelPakejDropdown = toggleHotelPakejDropdown_V114;

function closeHotelPakejDropdown_V114(roomId){
  const drop = document.getElementById('hotelPakejDrop-'+roomId);
  if(drop) drop.classList.add('hidden');
  setTimeout(()=>{ if(typeof renderRoomingGrid==='function') renderRoomingGrid(); }, 10);
}
window.closeHotelPakejDropdown = closeHotelPakejDropdown_V114;

function selectHotelPakej_V114(roomId, pakej){
  console.log('V114 selectHotelPakej', roomId, pakej);
  const room = (window.allRoomingRecords||[]).find(r=>r.id===roomId);
  if(room) room.fields['PAKEJ / HOTEL']=pakej;
  // Update button immediately - FIX for bug where box still shows EKONOMI
  const container = document.getElementById('hotelPakejContainer-'+roomId);
  if(container){
    const color = PAKEJ_COLORS_V114[pakej] || '#DBEAFE';
    ...
    if(btn){
      ...
      if(span) span.textContent=pakej;
      console.log('V114 hotel button updated to', pakej, color);
    }
    // Keep dropdown open
    const drop = document.getElementById('hotelPakejDrop-'+roomId);
    if(drop) drop.classList.remove('hidden');
    // Also update dropdown checkmarks
    if(drop){
      drop.querySelectorAll('div[onclick*="selectHotelPakej"]').forEach(div=>{
        const isSel = div.getAttribute('onclick')?.includes(`'${pakej}'`);
        const checkSpan = div.querySelector('span.w-4');
        if(checkSpan) checkSpan.textContent = isSel ? '✓' : '';
        const rightSpan = div.querySelector('span.ml-auto');
        if(rightSpan) rightSpan.textContent = isSel ? '✓' : '×';
        if(isSel) div.classList.add('bg-slate-100','font-bold');
  ...
  if(base&&pat&&room){
    ...
    }).then(r=>console.log('V114 hotel pakej saved', r.status, pakej));
  }
}
window.selectHotelPakej = selectHotelPakej_V114;

function clearHotelPakej_V114(roomId){
  const room = (window.allRoomingRecords||[]).find(r=>r.id===roomId);
...
window.clearHotelPakej = clearHotelPakej_V114;

// Outside click
document.removeEventListener('click', window._v114Outside);
window._v114Outside = function(e){
  const isBoardBtn = e.target.closest('button[onclick*="toggleBoardDropdown_V114"]');
  const isPakejBtn = e.target.closest('button[onclick*="togglePakejDropdown_V114"]');
  const isHotelBtn = e.target.closest('button[onclick*="toggleHotelPakejDropdown_V114"]');
...
document.addEventListener('click', window._v114Outside);

// Override renderNamelist
const origRenderNamelist_V114 = window.renderNamelist;
window.renderNamelist = function(){
  try{
    (window.allRoomingJemaah||[]).forEach(j=>{
      if(!j.fields) j.fields={};
      j.fields._jemaahId = j.id;
      const fb = getBoardFallback_V114(j.id);
      if(fb && fb.length && (!j.fields['BOARD BASIS'] || j.fields['BOARD BASIS'].length===0)){
        j.fields['BOARD BASIS']=fb;
      }
    });
  }catch(e){}
  const cont=document.getElementById('namelistContainer'); if(!cont) return;
  ...
  cont.innerHTML=filtered.map((r,i)=>{
    ...
    const fbArr = typeof getBoardArray==='function'? getBoardArray(r.fields) : (r.fields['BOARD BASIS']||[]);
    const fbDisplay = fbArr.length ? fbArr.join(', ') : '-';
    let fbCls='bg-white border-slate-200';
    if(fbArr.some(x=>x.includes('MEKAH'))) fbCls='bg-orange-100 border-orange-200 text-orange-800';
    else if(fbArr.some(x=>x.includes('MADINAH'))) fbCls='bg-blue-100 border-blue-200 text-blue-800';
    ...
    const boardCheckboxes = boardOptions.map(opt=>{
      const checked=fbArr.includes(opt);
      return `<label class="flex items-center gap-1.5 px-2 py-1 hover:bg-slate-50 rounded text-[10px] cursor-pointer" onclick="event.stopPropagation()"><input type="checkbox" ${checked?'checked':''} onchange="toggleBoardMulti_V114('${r.id}','${opt}')" class="w-3 h-3 accent-[#7A0C2E]"> ${opt}</label>`;
    }).join('');
    ...
    const pkColor = PAKEJ_COLORS_V114[pk] || '#FFFFFF';
    const pkDisplay = pk || '-';
    const pakejDropdown = `<div class="relative"><button onclick="event.stopPropagation(); togglePakejDropdown_V114('${r.id}')" class="w-full text-[9px] border rounded-full px-2.5 py-1.5 font-bold text-left flex items-center justify-between" style="background:${pkColor}; border-color:${pkColor}; color:#1F2937"><span class="truncate">${pkDisplay}</span><span>▼</span></button><div id="pakejDrop-${r.id}" class="hidden absolute z-[9999] mt-1 w-48 bg-white border border-slate-300 rounded-xl shadow-2xl p-1" onclick="event.stopPropagation()">${PAKEJ_ORDER_V114.map(o=>{
      const c = PAKEJ_COLORS_V114[o];
      const isSel = pk===o;
      return `<div onclick="selectPakej_V114('${r.id}','${o}')" class="flex items-center gap-2 px-2.5 py-1.5 hover:bg-slate-50 cursor-pointer text-[11px] rounded-lg ${isSel?'bg-slate-100 font-bold':''}"><span class="w-4 h-4 rounded-full flex items-center justify-center text-[9px] border" style="background:${c}; border-color:${c}">${isSel?'✓':''}</span> ${o} <span class="ml-auto text-slate-400">${isSel?'✓':'×'}</span></div>`;
    }).join('')}<div class="flex justify-between gap-1 mt-1 pt-1 border-t bg-white"><button onclick="clearPakej_V114('${r.id}'); event.stopPropagation();" class="text-[9px] px-3 py-1 rounded-full bg-slate-100">Clear</button><button onclick="closePakejDropdown_V114('${r.id}'); event.stopPropagation();" class="text-[9px] px-3 py-1 rounded-full bg-[#7A0C2E] text-white">OK</button></div></div></div>`;
    const trChecked = typeof isTrainChecked==='function'? isTrainChecked(r.fields) : false;
    return `<div ${drag} class="grid grid-cols-12 items-center px-1.5 py-1.5 text-[11px] border-b border-slate-50 ${rowCls}">
      <div class="col-span-1 text-slate-400 text-[10px]">${String(i+1).padStart(2,'0')}</div>
      <div class="col-span-3 font-medium truncate text-[10px] ${assignedInLoc?'text-slate-500 italic':''}" title="${name}">${name}</div>
      <div class="col-span-2 flex items-center gap-0.5 relative">
        <div class="relative w-full">
          <button onclick="event.stopPropagation(); toggleBoardDropdown_V114('${r.id}')" class="text-[8px] border rounded-full px-2 py-1 font-bold ${fbCls} outline-none w-full truncate text-left flex items-center justify-between bg-white">
            <span class="truncate">${fbDisplay}</span><span class="ml-1">▼</span>
          </button>
          <div id="boardDrop-${r.id}" class="hidden absolute left-0 top-full mt-1 w-[190px] bg-white border border-slate-200 rounded-xl shadow-xl z-[9999] p-1" onclick="event.stopPropagation()">
            ${boardCheckboxes}
            <div class="border-t border-slate-100 mt-1 pt-1 flex justify-between">
              <button onclick="clearBoardMulti_V114('${r.id}'); event.stopPropagation();" class="text-[8px] px-2 py-0.5 rounded-full bg-slate-100">Clear</button>
...
// Hotel grid with color dropdown
const origRenderRoomingGrid_V114 = window.renderRoomingGrid;
window.renderRoomingGrid = function(){
  if(origRenderRoomingGrid_V114) origRenderRoomingGrid_V114();
  setTimeout(()=>{
