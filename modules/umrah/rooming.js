    ...
    <div class="bg-white rounded-2xl p-5 max-w-sm w-full shadow-2xl">
      ...
      <div class="space-y-3 text-xs">
        ...
        <div class="flex gap-2 pt-2">
          ...
          <button onclick="submitNewRoom()" id="btnCiptaBilik" class="flex-1 py-2.5 bg-slate-900 text-white rounded-xl font-bold hover:bg-black">Cipta Bilik</button>
        </div>
      </div>
    </div>
  </div>
...
async function fetchRoomingData(){
  try{
    ...
    if(!tripId){ document.getElementById('namelistContainer').innerHTML='<div class="p-8 text-center text-xs text-slate-400">Pilih Trip dulu</div>'; return; }
...
function renderNamelist(){
  ...
  document.getElementById('totalJemaahBadge').textContent=total+' Total'; document.getElementById('belumAssignBadge').textContent=belum+' Unassigned';
  document.getElementById('belumAssignTop').textContent=belum+' Unassigned'; document.getElementById('assignedTop').textContent=(total-belum)+' Assigned';
...
async function submitNewRoom(){
  ...
  try{
    ...
    if(newRec.id){ allRoomingRecords.push(newRec); closeNewRoomModal(); renderRoomingGrid(); renderLocationTabs(); document.getElementById('newRoomHotel').value=''; document.getElementById('newRoomNote').value=''; }
...
function openCopyRoomsModal(){
  ...
  list.innerHTML=allLocs.map(loc=>`<label class="flex items-center gap-2 p-2 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-50"><input type="radio" name="copySource" value="${loc}"><span class="text-xs font-bold">📍 ${loc} (${counts[loc]||0} bilik)</span></label>`).join('')||'<div class="text-xs text-slate-400">Tiada lokasi</div>';
...
async function executeCopyRooms(){
  ...
  for(let r of srcRooms){
    ...
    try{
      let res=await fetch(`https://api.airtable.com/v0/${base}/ROOMING%20LIST`,{method:'POST',headers:{Authorization:`Bearer ${pat}`,'Content-Type':'application/json'},body:JSON.stringify(payload)});
      let newRec=await res.json();
      if(newRec.error && newRec.error.message.includes('not writable')){ delete payload.fields['Room ID / Nama Bilik']; const res2=await fetch(`https://api.airtable.com/v0/${base}/ROOMING%20LIST`,{method:'POST',headers:{Authorization:`Bearer ${pat}`,'Content-Type':'application/json'},body:JSON.stringify(payload)}); newRec=await res2.json(); }
      if(newRec.id){ allRoomingRecords.push(newRec); created++; }
    }catch(e){ console.error(e); }
  }
  closeCopyRoomsModal(); renderRoomingGrid(); renderLocationTabs(); alert(`Berjaya copy ${created} bilik ke ${activeLocation}`);
}
