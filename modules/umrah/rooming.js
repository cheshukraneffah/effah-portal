// ROOMING V25.1 LIGHT - Keep all features (drag room, drag jemaah, staff, tanpa katil, copy, autoAssign, print highlight) + fix infant remove + perf batchRender still editable + print highlight (TAKAFUL hijau, ETIQA kuning, KHAIRI biru, TRAIN kuning)
...
async async async function fetchRoomingData(){
  try{
...
function batchRenderRooming(){
  clearTimeout(_renderDebounce);
  _renderDebounce=setTimeout(()=>{ renderRoomingGrid(); renderNamelist(); renderStaffList(); renderLocationTabs(); }, 120);
}
async function updateRoomField(roomId,field,value,doRender=true){
  ...
  try{
    ...
    if(doRender){ batchRenderRooming(); }
...
function saveStaffList(){ localStorage.setItem(getStaffStorageKey(),JSON.stringify(staffList)); try{ localStorage.setItem('effah_staff_board_'+activeLocation, JSON.stringify(staffList)); }catch(e){} }
