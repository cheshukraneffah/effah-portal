...
// V112 - FIX CLEAR BUTTON + PAKEJ COLOR SINGLE SELECT + OUTSIDE CLICK

const PAKEJ_ORDER_V112 = ['JIMAT EKONOMI','JIMAT STANDARD','JIMAT PREMIUM','EKONOMI LITE','EKONOMI','STANDARD','PREMIUM','PREMIUM PLUS'];
const PAKEJ_COLORS_V112 = {
...
function closeAllDropdowns_V112(){
...
function toggleBoardDropdown_V112(jId){
  const drop = document.getElementById('boardDrop-'+jId);
  if(!drop) return;
  const isHidden = drop.classList.contains('hidden');
  closeAllDropdowns_V112();
...
window.toggleBoardDropdown = toggleBoardDropdown_V112;
...
function toggleBoardMulti_V112(jId, boardVal){
  try{
    ...
    if(jRec){
      ...
      setBoardFallback_V112(jId, jRec.fields['BOARD BASIS']);
      const drop = document.getElementById('boardDrop-'+jId);
      if(drop){
      ...
      document.querySelectorAll(`button[onclick*="toggleBoardDropdown_V112('${jId}')"]`).forEach(btn=>{
      ...
      if(base&&pat){
        fetch(`https://api.airtable.com/v0/${base}/JEMAAH/${jId}`,{
          ...
          console.log('V112 board toggle', r.status, jRec.fields['BOARD BASIS']);
          if(r.ok){ try{ localStorage.removeItem('effah_board_fallback_'+jId); }catch(e){} }
          else {
            const j = await r.json().catch(()=>({}));
            console.error('Board save error', r.status, j.error?.message);
          }
        });
      }
    } else {
      ...
      if(sRec){
        ...
        if(base&&pat&&sRec.airtableId){
          ...
          }).then(r=>console.log('V112 staff board', r.status));
        }
      }
    }
  }catch(e){ console.error(e); }
}
window.toggleBoardMulti = toggleBoardMulti_V112;

// CLEAR FIX - this was not working
function clearBoardMulti_V112(jId){
  console.log('V112 clearBoardMulti called', jId);
  try{
    const jRec = (window.allRoomingJemaah||[]).find(j=>j.id===jId);
    if(jRec){
      if(!jRec.fields) jRec.fields={};
    ...
    setBoardFallback_V112(jId, []);
    const drop = document.getElementById('boardDrop-'+jId);
    if(drop){
      drop.querySelectorAll('input[type="checkbox"]').forEach(cb=>cb.checked=false);
      drop.classList.remove('hidden'); // stay open as per request
    }
    // Update button text to -
    const btns = document.querySelectorAll(`button[onclick*="toggleBoardDropdown_V112('${jId}')"]`);
    btns.forEach(btn=>{
      const span = btn.querySelector('span.truncate');
    ...
    if(base&&pat){
      ...
      fetch(`https://api.airtable.com/v0/${base}/${table}/${idToPatch}`,{
        ...
        console.log('V112 clear board', r.status);
        if(!r.ok){
          const j = await r.json().catch(()=>({}));
          console.error('Clear board error', r.status, j.error?.message);
        }
      });
    }
  }catch(e){ console.error('clearBoard error', e); }
}
window.clearBoardMulti = clearBoardMulti_V112;

// PAKEJ - COLOR SINGLE SELECT (like image 2)
function togglePakejDropdown_V112(jId){
...
window.togglePakejDropdown = togglePakejDropdown_V112;

function closePakejDropdown_V112(jId){
  const drop = document.getElementById('pakejDrop-'+jId);
  if(drop) drop.classList.add('hidden');
  setTimeout(()=>{ if(typeof renderNamelist==='function') renderNamelist(); }, 10);
}
window.closePakejDropdown = closePakejDropdown_V112;

function selectPakej_V112(jId, pakej){
  const jRec = (window.allRoomingJemaah||[]).find(j=>j.id===jId);
  if(jRec) jRec.fields['PAKEJ']=pakej;
  const drop = document.getElementById('pakejDrop-'+jId);
  const wasOpen = drop && !drop.classList.contains('hidden');
  // Don't full re-render yet, just update button color
  if(drop){
    const btn = document.querySelector(`button[onclick*="togglePakejDropdown_V112('${jId}')"]`);
    if(btn){
      const color = PAKEJ_COLORS_V112[pakej] || '#FFFFFF';
      btn.style.background = color;
      btn.style.borderColor = color;
      const span = btn.querySelector('span.truncate');
      if(span) span.textContent = pakej;
    }
    drop.classList.remove('hidden'); // stay open until OK or outside
  }
  const base=window.AIRTABLE_BASE_ID||localStorage.getItem('effah_api_base');
  const pat=window.AIRTABLE_PAT||localStorage.getItem('effah_api_pat');
  if(base&&pat&&jRec){
    ...
    }).then(r=>console.log('V112 pakej saved', r.status, pakej));
  }
}
window.selectPakej = selectPakej_V112;

function clearPakej_V112(jId){
  const jRec = (window.allRoomingJemaah||[]).find(j=>j.id===jId);
...
window.clearPakej = clearPakej_V112;

// HOTEL PAKEJ - same colors
function toggleHotelPakejDropdown_V112(roomId){
...
window.toggleHotelPakejDropdown = toggleHotelPakejDropdown_V112;

function closeHotelPakejDropdown_V112(roomId){
  const drop = document.getElementById('hotelPakejDrop-'+roomId);
  if(drop) drop.classList.add('hidden');
  setTimeout(()=>{ if(typeof renderRoomingGrid==='function') renderRoomingGrid(); }, 10);
}
window.closeHotelPakejDropdown = closeHotelPakejDropdown_V112;

function selectHotelPakej_V112(roomId, pakej){
  const room = (window.allRoomingRecords||[]).find(r=>r.id===roomId);
  if(room) room.fields['PAKEJ / HOTEL']=pakej;
  const drop = document.getElementById('hotelPakejDrop-'+roomId);
  if(drop) drop.classList.remove('hidden');
  const btn = document.querySelector(`button[onclick*="toggleHotelPakejDropdown_V112('${roomId}')"]`);
  if(btn){
    const color = PAKEJ_COLORS_V112[pakej] || '#FFFFFF';
    btn.style.background=color;
    btn.style.borderColor=color;
  ...
  if(base&&pat&&room){
    ...
    }).then(r=>console.log('V112 hotel pakej saved', r.status));
  }
}
window.selectHotelPakej = selectHotelPakej_V112;

// Outside click auto collapse
document.removeEventListener('click', window._v112Outside);
window._v112Outside = function(e){
  const isBoardBtn = e.target.closest('button[onclick*="toggleBoardDropdown_V112"]');
  const isPakejBtn = e.target.closest('button[onclick*="togglePakejDropdown_V112"]');
  const isHotelBtn = e.target.closest('button[onclick*="toggleHotelPakejDropdown_V112"]');
  ...
  if(!isBoardBtn && !isPakejBtn && !isHotelBtn && !isInsideBoard && !isInsidePakej && !isInsideHotel){
    const openBoard = document.querySelector('[id^="boardDrop-"]:not(.hidden)');
    const openPakej = document.querySelector('[id^="pakejDrop-"]:not(.hidden)');
    const openHotel = document.querySelector('[id^="hotelPakejDrop-"]:not(.hidden)');
    if(openBoard || openPakej || openHotel){
      closeAllDropdowns_V112();
      setTimeout(()=>{ if(typeof renderNamelist==='function') renderNamelist(); if(typeof renderRoomingGrid==='function') renderRoomingGrid(); }, 10);
    }
  }
};
document.addEventListener('click', window._v112Outside);

// Override renderNamelist to include pakej color dropdown + board with fallback
const origRenderNamelist_V112 = window.renderNamelist;
window.renderNamelist = function(){
  try{
    (window.allRoomingJemaah||[]).forEach(j=>{
      if(!j.fields) j.fields={};
      j.fields._jemaahId = j.id;
      const fb = getBoardFallback_V112(j.id);
  ...
  cont.innerHTML=filtered.map((r,i)=>{
    ...
    const boardCheckboxes = boardOptions.map(opt=>{
      const checked=fbArr.includes(opt);
      return `<label class="flex items-center gap-1.5 px-2 py-1 hover:bg-slate-50 rounded text-[10px] cursor-pointer" onclick="event.stopPropagation()"><input type="checkbox" ${checked?'checked':''} onchange="toggleBoardMulti_V112('${r.id}','${opt}')" class="w-3 h-3 accent-[#7A0C2E]"> ${opt}</label>`;
    }).join('');
    ...
    const pkColor = PAKEJ_COLORS_V112[pk] || '#FFFFFF';
    const pkDisplay = pk || '-';
    const pakejDropdown = `<div class="relative"><button onclick="event.stopPropagation(); togglePakejDropdown_V112('${r.id}')" class="w-full text-[9px] border rounded-full px-2.5 py-1.5 font-bold text-left flex items-center justify-between" style="background:${pkColor}; border-color:${pkColor}; color:#1F2937"><span class="truncate">${pkDisplay}</span><span>▼</span></button><div id="pakejDrop-${r.id}" class="hidden absolute z-[9999] mt-1 w-48 bg-white border border-slate-300 rounded-xl shadow-2xl p-1" onclick="event.stopPropagation()">${PAKEJ_ORDER_V112.map(o=>{
      const c = PAKEJ_COLORS_V112[o];
      const isSel = pk===o;
      return `<div onclick="selectPakej_V112('${r.id}','${o}')" class="flex items-center gap-2 px-2.5 py-1.5 hover:bg-slate-50 cursor-pointer text-[11px] rounded-lg ${isSel?'bg-slate-100 font-bold':''}"><span class="w-4 h-4 rounded-full flex items-center justify-center text-[9px] border" style="background:${c}; border-color:${c}">${isSel?'✓':''}</span> ${o} <span class="ml-auto text-slate-400">${isSel?'✓':'×'}</span></div>`;
    }).join('')}<div class="flex justify-between gap-1 mt-1 pt-1 border-t bg-white"><button onclick="clearPakej_V112('${r.id}'); event.stopPropagation();" class="text-[9px] px-3 py-1 rounded-full bg-slate-100">Clear</button><button onclick="closePakejDropdown_V112('${r.id}'); event.stopPropagation();" class="text-[9px] px-3 py-1 rounded-full bg-[#7A0C2E] text-white">OK</button></div></div></div>`;
    const trChecked = typeof isTrainChecked==='function'? isTrainChecked(r.fields) : false;
    return `<div ${drag} class="grid grid-cols-12 items-center px-1.5 py-1.5 text-[11px] border-b border-slate-50 ${rowCls}">
      <div class="col-span-1 text-slate-400 text-[10px]">${String(i+1).padStart(2,'0')}</div>
      <div class="col-span-3 font-medium truncate text-[10px] ${assignedInLoc?'text-slate-500 italic':''}" title="${name}">${name}</div>
      <div class="col-span-2 flex items-center gap-0.5 relative">
        <div class="relative w-full">
          <button onclick="event.stopPropagation(); toggleBoardDropdown_V112('${r.id}')" class="text-[8px] border rounded-full px-2 py-1 font-bold ${fbCls} outline-none w-full truncate text-left flex items-center justify-between bg-white" style="opacity:1;">
            <span class="truncate">${fbDisplay}</span><span class="ml-1">▼</span>
          </button>
          <div id="boardDrop-${r.id}" class="hidden absolute left-0 top-full mt-1 w-[190px] bg-white border border-slate-200 rounded-xl shadow-xl z-[9999] p-1" onclick="event.stopPropagation()">
            ${boardCheckboxes}
            <div class="border-t border-slate-100 mt-1 pt-1 flex justify-between">
              <button onclick="clearBoardMulti_V112('${r.id}'); event.stopPropagation();" class="text-[8px] px-2 py-0.5 rounded-full bg-slate-100">Clear</button>
...
// Also patch hotel pakej after grid render
const origRenderRoomingGrid_V112 = window.renderRoomingGrid;
window.renderRoomingGrid = function(){
  if(origRenderRoomingGrid_V112) origRenderRoomingGrid_V112();
  setTimeout(()=>{
