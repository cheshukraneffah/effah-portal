...
function renderRoomingHTML(){
      ...
      <div class="w-full xl:w-[52%] bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col overflow-hidden">
        ...
        <div class="border-t border-slate-200 bg-amber-50/30">
          ...
          <div class="px-3 pb-3 flex gap-2">
            ...
            <button onclick="addNewStaff()" class="px-3 py-2 bg-[#881337] hover:bg-[#9f1239] text-white rounded-xl text-xs font-bold">+ Add</button>
          </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-3">
          <div class="flex items-center justify-between gap-2 flex-wrap">
            ...
            <div class="flex items-center gap-1.5 flex-wrap">
              ...
              <button onclick="autoAssignRooming()" class="px-3 py-1.5 bg-slate-100 border border-slate-200 text-slate-700 hover:bg-slate-50 rounded-full text-xs font-bold">Auto Assign</button>
              <button onclick="openNewRoomModal()" class="px-3 py-1.5 bg-[#881337] hover:bg-[#9f1239] text-white border border-[#881337] rounded-full text-xs font-bold">+ Bilik Baru</button>
            </div>
          </div>
  ...
  <div id="newRoomModal" class="fixed inset-0 bg-black/50 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl p-5 max-w-sm w-full shadow-2xl">
      ...
      <div class="space-y-3 text-xs">
        ...
        <div class="flex gap-2 pt-2"><button onclick="closeNewRoomModal()" class="flex-1 py-2.5 bg-slate-100 rounded-xl font-bold">Batal</button><button onclick="submitNewRoom()" class="flex-1 py-2.5 bg-white border border-slate-200 text-slate-800 rounded-xl font-bold">Cipta Bilik</button></div>
      </div>
    </div>
  </div>
  ...
  <div id="copyRoomsModal" class="fixed inset-0 bg-black/50 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl p-5 max-w-sm w-full shadow-2xl">
      ...
      <div class="flex gap-2"><button onclick="closeCopyRoomsModal()" class="flex-1 py-2.5 bg-slate-100 rounded-xl font-bold text-xs">Batal</button><button onclick="executeCopyRooms()" class="flex-1 py-2.5 bg-white border border-slate-200 text-slate-800 rounded-xl font-bold text-xs">Copy Sekarang</button></div>
    </div>
  </div>
  `;
  ...
  renderLocationTabs();
  fetchRoomingData();
  setTimeout(enableDragAutoScroll, 500);
}
...
function enableDragAutoScroll(){
  ...
  document.addEventListener('dragover', (e)=>{
    ...
    if(y < threshold){
      if(!scrollInterval) scrollInterval=setInterval(()=>{ window.scrollBy(0,-14); document.getElementById('roomingGrid')?.scrollBy(0,-14); document.getElementById('namelistContainer')?.scrollBy(0,-14); document.getElementById('staffListContainer')?.scrollBy(0,-14); },16);
    } else if(y > h - threshold){
      if(!scrollInterval) scrollInterval=setInterval(()=>{ window.scrollBy(0,14); document.getElementById('roomingGrid')?.scrollBy(0,14); },16);
    } else {
...
document.addEventListener('DOMContentLoaded', ()=>{ setTimeout(enableDragAutoScroll, 800); });
...
function generateRoomingPrint(){
  ...
  // clean helper
  function cleanNameForPrint(raw){
    if(!raw) return '';
  ...
  // NAMELIST: jemaah only numbered, staff as NA/S
  let namelistRows='';
  let jNo=1;
  allRoomingJemaah.forEach(j=>{
    const raw=j.fields['NAMA']||'-';
    // skip staff names that look like staff? Keep all but separate numbering
    if(String(raw).toUpperCase().includes('(EFFAH)') || String(raw).toUpperCase().includes('MUTAWIF')) return;
    const name=cleanNameForPrint(raw);
    const ejen=j.fields['EJEN']||'-';
    const board=j.fields['BOARD']||'-';
    const pakej=j.fields['PAKEJ']||'EKONOMI';
    namelistRows+=`<tr><td style="text-align:center;">${jNo}</td><td>${name}</td><td>${ejen}</td><td>${board}</td><td>-</td><td>${pakej}</td><td></td></tr>`;
    jNo++;
  });
  // Staff from staffList + extra in rooms
  const allStaffNames = [...new Set([...staffList.map(s=>s.name), ...rooms.flatMap(r=> (r.fields['STAFF / EXTRA']||'').split(',').map(x=>x.trim()).filter(Boolean))])];
  allStaffNames.forEach(sName=>{
    const clean=cleanNameForPrint(sName.replace(/\(EFFAH\)/i,'').trim()) + ' (EFFAH)';
    namelistRows+=`<tr><td style="text-align:center; background:#fff1f2; color:#881337; font-weight:bold;">NA</td><td style="background:#fff1f2; color:#881337;">${clean}</td><td style="background:#fff1f2;">-</td><td style="background:#fff1f2;">-</td><td style="background:#fff1f2;">-</td><td style="background:#fff1f2;">-</td><td style="background:#fff1f2;">-</td></tr>`;
  });

  // ROOM BLOCKS: numbering only jemaah, staff not numbered
  const roomBlocks=rooms.map(r=>{
    ...
    let num=1;
    jIds.forEach(jId=>{
      const rec=allRoomingJemaah.find(j=>j.id===jId); const raw=rec?.fields?.['NAMA']||'';
      const name=cleanNameForPrint(raw);
      if(!name) return;
      rows+=`<div>${num}. ${name}</div>`;
      num++;
    });
    staff.forEach(s=>{
      const clean=cleanNameForPrint(s.replace(/\(EFFAH\)/i,'').trim());
      // NA in number column logic: show as STAFF line without number, or NA prefix in number column
      rows+=`<div style="color:#881337; margin-top:2px;">STAFF: ${clean} (EFFAH)</div>`;
    });
    return `<div style="margin-bottom:18px; page-break-inside:avoid;"><b>${rid} (${pakej})${hotel?' - '+hotel:''}</b><div style="margin-left:8px; margin-top:4px; line-height:1.6; font-size:10px;">${rows||'- Kosong -'}</div></div>`;
  }).join('');

  const html=`<html><head><title>Rooming ${tripName} - ${activeLocation}</title><style>
  body{font-family:Arial;font-size:10px;margin:15px}
  table{border-collapse:collapse;width:100%}th,td{border:1px solid #000;padding:4px 6px;font-size:9px}
  th{background:#f1f5f9; font-weight:bold}
  .header{display:flex;justify-content:space-between;font-weight:bold;font-size:13px;border-bottom:2px solid #000;padding-bottom:6px;margin-bottom:12px}
  .container{display:flex;gap:15px} .left{width:68%} .right{width:32%;border-left:1px solid #000;padding-left:10px}
  @media print{ @page{size:A4 landscape; margin:10mm} }
  </style></head><body>
  <div class="header"><span>NAMELIST ${tripName}</span><span>ROOMING LIST ${tripName} - ${activeLocation}</span></div>
  <div class="container"><div class="left"><table><tr><th>NO</th><th>NAMA JEMAAH</th><th>EJEN</th><th>FULLBOARD</th><th>TRAIN</th><th>PAKEJ</th><th>INSURAN</th></tr>${namelistRows}</table></div><div class="right">${roomBlocks}</div></div>
  <script>window.onload=function(){setTimeout(()=>window.print(),400)}</script></body></html>`;
